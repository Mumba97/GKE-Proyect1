// --- CONFIGURACIÓN ---
// Se configura con los datos de User Pool y API Gateway para que la app funcione
const config = {
    cognito: {
        userPoolId: 'us-east-1_TQa3fq3Wo', // Se coloca el ID del User Pool configurado en AWS Cognito
        userPoolWebClientId: '2d95kbrgtmb8uaqvlnijvrp95o', // Se coloca el App Client ID del User Pool
        region: 'us-east-1' // La región en la que estamos trabajando
    },
    api: {
        invokeUrl: 'https://s0o26sqlv6.execute-api.us-east-1.amazonaws.com/v1', // La URL de despliegue de  API Gateway
        apiKey: 'o0bOCLCD7l6nxm9lA8YRz4pTR9V40WHxaecYTLvl' // La API Key (solo para la alerta, pero la guardamos aquí por si acaso)
    }
};
...
const poolData = {
    UserPoolId: config.cognito.userPoolId, // sirve para identificar el User Pool
    ClientId: config.cognito.userPoolWebClientId, // sirve para identificar la app dentro del User Pool
};
const userPool = new AmazonCognitoIdentity.CognitoUserPool(poolData); //sirve para manejar el User Pool
let currentUserToken = null; // Aquí guardaremos el token JWT del usuario autenticado

// Helper para mostrar mensajes al usuario
function showStatus(message, isSuccess) { //esta función muestra mensajes de éxito o error en la UI
...
async function apiCall(endpoint, method, body) {
    ...
    const res = await fetch(config.api.invokeUrl + endpoint, {//se hace la llamada a la API
        method,
        headers: {
...
function signIn() {//esta función maneja el inicio de sesión de usuarios con Cognito
...
function signOut() {//esta función maneja el cierre de sesión de usuarios con Cognito
...
//Esta Funcion nos sirve para CONFIRMAR REGISTRO
...
// Esta funcion nos sirve par ALTERNAR FORMULARIOS DE LOGIN/REGISTRO
...
async function getDeviceStatus() { //esta función obtiene el estado del dispositivo pareado
    try {
...
async function pairDevice() { //esta función maneja el pareo de un dispositivo con el usuario
    const deviceId = document.getElementById('deviceId-input').value;
    if (!deviceId) {
        showStatus('Por favor, ingresa el ID de tu dispositivo.', false);
        return;
    }
    try {
...
async function getContacts() {//esta función obtiene y muestra la lista de contactos de emergencia del usuario
...
async function addContact() {//esta función maneja la adición de un nuevo contacto de emergencia
...
async function deleteContact(contactId) {//esta función maneja la eliminación de un contacto de emergencia
...
// --- Inicialización ---
//esta función verifica si el usuario ya tiene una sesión activa al cargar la página
